...
def list_positions_assigned_for_a_school(school_code: int):
    return [SchoolAssignment(position=Position(code=row[3], name=row[4], corps=row[5]), quantity=row[2])
            for row in
...
def list_schools_with_specific_position_assigned(position_code: str):
    return [PositionAssignment(school=School(code=row[3], name=row[4], province=row[5], locality=row[6]), quantity=row[2])
            for row in
